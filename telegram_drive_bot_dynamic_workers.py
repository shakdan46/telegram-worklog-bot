...
async def handle_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        workers = fetch_workers_by_date(FILE_ID, date)
        if not workers:
            await update.message.reply_text("⚠️ לא נמצאו פועלים בתאריך הזה.")
            return SELECTING_DATE

        keyboard = [[InlineKeyboardButton(name, callback_data=name)] for name in workers]
...
def fetch_workers_by_date(file_id, date):
    try:
        ...
        filtered = df[df["תאריך"] == date_obj]
        workers = filtered["שם הפועל"].dropna().unique().tolist()
        return workers
    except Exception as e:
        print(f"שגיאה בקריאת שמות הפועלים: {e}")
        return []

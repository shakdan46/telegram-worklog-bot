...
def upload_excel(file_path):
    ...
    drive_service.files().update(fileId=SPREADSHEET_ID, media_body=media_body).execute()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_authorized(user_id):
        ...
        return ASK_NAME  # נשתמש בשלב קיים כסיסמה
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            if row[0] and isinstance(row[0], datetime):
                if row[0].strftime('%d/%m/%Y') == context.user_data['selected_date'] and row[1]:
...
async def handle_worker_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "done":
        ...
        text = "נבחרו:\n" + "\n".join(selected)
        buttons = [[InlineKeyboardButton("אישור ✅", callback_data="confirm")],
...
async def confirm_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "remove":
        ...
        buttons = [[InlineKeyboardButton(name, callback_data=f"remove_{name}")] for name in selected]
        buttons.append([InlineKeyboardButton("ביטול 🚫", callback_data="confirm")])
...
async def start_add_worker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("הזן את שם הפועל:")
    return ASK_NAME
